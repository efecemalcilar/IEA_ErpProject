...
namespace IEA_ErpProject.BilgiGiris.Doktorlar
{
    ...
    public partial class DoktorGiris : Form
    {
        ...
        {
           // TxtUnvan.DataSource = Enum.GetValues(typeof(Unvan));            //Enum bana bir veri getirsin istiyorum sonra tipi typeof, oluşturduğum enum class ismini(Unvan) ekleyip ; koycaz
            ...
            TxtUnvan.DataSource = Enum.GetValues(typeof(Unvan)).Cast<Enum>()
                ...
                .ToList();                     //TxtUnvan.data diye başlayıp Buraya kadar olan kısım unvan kısmındaki boşluklar içindir

            TxtUnvan.DisplayMember = "Description";
            TxtUnvan.ValueMember = "Value";
            // Başlangıcta seçilen yerlerin boş gelmesini sağlar.
        ...
        private List<tblDoktorlar> dktList;        // tblDoktorlar listesinde ki elemanları liste oluşturarak dktList e at
        ...
        public void Ac(int secimId) //parantez icinde id yaziyordu
        {
            Temizle();
            if (secimId < 0)
            {
            ...
            kayitBul = dktList.FirstOrDefault(x => x.Id == secimId); // Bu db ye bağlanmaz x dktList den bir nesne olarak türetilir . amacımız dktList deki columb bilgilerine ulaşabilmek.  (x => x.Id == secimId)  x i oluşturdum x den id yi türettim... Lambda expression
